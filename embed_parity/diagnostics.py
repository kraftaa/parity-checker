...
def diagnose(report: dict[str, Any]) -> list[dict[str, str]]:
        ...
        findings.append(
            {
                ...
                "message": (
                    f"Runtime dtypes differ ({metadata['reference_dtype']} vs "
                    f"{metadata['candidate_dtype']}); "
    ...
    if not structural["same_output_count"]:
        findings.append(
            {"code": "output_count", "message": "Output counts differ between runtimes."}
        )
        ...
        findings.append(
            {
                ...
                "message": (
                    "Possible pooling, projection, model, or export mismatch; "
    ...
    if not structural["finite_vectors"]:
        findings.append(
            {"code": "non_finite", "message": "One or both runtimes returned NaN or Inf values."}
        )
    if structural["reference"]["zero_vectors"] or structural["candidate"]["zero_vectors"]:
        findings.append(
            {"code": "zero_vectors", "message": "One or both runtimes returned zero vectors."}
        )
        ...
        findings.append(
            {
                ...
                "message": (
                    "Possible normalization mismatch: directions agree but vector norm "
    ...
    if prefix and prefix["possible_mismatch"]:
        affected = [
            ...
            if prefix.get(name) and prefix[name]["possible_mismatch"]
        ]
        findings.append(
            {
                "code": "query_prefix_mismatch",
                "message": (
                    f"A model-recommended prefix substantially improves cross-runtime parity for "
                    f"{' and '.join(affected)} probes."
                ),
            }
        )
        ...
        findings.append(
            {
                ...
                "message": (
                    "Possible truncation or maximum-sequence-length mismatch; short inputs "
                    "pass while longer inputs diverge."
                ),
            }
        )
    ...
    if (
        ...
        and not any(
            item["code"] in {"normalization_mismatch", "query_prefix_mismatch", "length_boundary"}
        ...
        findings.append(
            {
                ...
                "message": (
                    "Embedding directions differ materially; possible revision, dtype, "
                    "preprocessing, or runtime mismatch."
                ),
            }
        )
    return findings
