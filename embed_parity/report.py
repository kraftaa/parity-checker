...
def render_text(report: dict[str, Any]) -> str:
    ...
    if report["vector"]:
        ...
        vector_pass = v["mean"] >= report["thresholds"]["vector_mean"] and v["minimum"] >= report["thresholds"]["vector_min"]
        geometry_pass = g["spearman"] >= report["thresholds"]["geometry_spearman"] and g["mean_absolute_difference"] <= report["thresholds"]["geometry_mae"]
        lines.extend([
            "", "Vector parity",
        ...
        lines.extend([
            ...
            f"                                   {_status(n['top_1_overlap'] >= report['thresholds']['top1_agreement'] and n['top_5_overlap'] >= report['thresholds']['top5_overlap'] and n['top_10_overlap'] >= report['thresholds']['top10_overlap'])}",
            "", "Neighborhoods changed most",
        ])
        for item in n["most_changed"]:
            lines.append(f"  {item['id']:<8} top-10 overlap {_fmt(item['top_10_overlap'] * 100, 1)}%")
    ...
    if batches:
        ...
        for key, label in (("reference", "SentenceTransformers"), ("candidate", "TEI")):
            ...
            for item in batches[key]["comparisons"]:
                lines.append(f"    batch {item['batch_a']} vs {item['batch_b']}: mean {_fmt(item['mean_cosine'])}  {_status(item['passed'])}")
    length = report.get("length_analysis")
    if length:
        lines.extend(["", "Input length parity"])
        for point in length["points"]:
            lines.append(f"  {point['tokens']:>4} tokens  {_fmt(point['cosine'])}  {_status(point['passed'])}")
        minimal = length.get("minimal_reproducer")
        if minimal:
            lines.extend([
                "", "Minimal reproducer",
                f"  original failing probe: {minimal['original_failing_tokens']} tokens",
                f"  {minimal['previous_tokens']} tokens: cosine = {_fmt(minimal['previous_cosine'])}",
    ...
    if prefix:
        lines.extend([
            ...
            f"  raw-to-raw                       {_fmt(prefix['scores']['raw_to_raw'])}",
            f"  best cross-variant improvement   {_fmt(prefix['cross_variant_improvement'])}",
        ])
...
def write_json(report: dict[str, Any], path: str | Path) -> None:
    Path(path).write_text(json.dumps(_json_safe(report), indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
